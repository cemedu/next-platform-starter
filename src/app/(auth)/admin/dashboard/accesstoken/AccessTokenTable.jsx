...
const AccessTokenTable = ({ rows = [], cols = [], token = '' }) => {
    const router = useRouter();

    const DeleteHandler = async (id) => {
        const { success, error, message } = await Actions.AccessToken.Delete(token, id);
        if (success) {
                ...
                <tbody>
                    {rows.map((x) => (
                        <tr key={x.id} className={x.paid && 'bg-green-600'}>
                            ...
                            <td>{`${new Date(x.expire).toLocaleDateString()}`}</td>
...
export default React.memo(AccessTokenTable);