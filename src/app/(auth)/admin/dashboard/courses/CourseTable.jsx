...
const CourseTable = ({ rows = [], cols = [], token }) => {
    ...
    const DeleteHandler = async (id) => {
        const { success, error, message } = await Actions.Course.Delete(token, id);
        if (success) {
    ...
    const EditHandler = async (id) => {
        router.push(`${NavLink.admin.courses}?id=update&p_id=${id}`);
    }

    const DetailsHandler = async (id) => {
        router.push(`${NavLink.courses}/${id}`);
    }
