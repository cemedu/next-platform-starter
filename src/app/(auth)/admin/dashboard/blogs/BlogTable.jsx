...
const BlogTable = ({ token, rows = [], cols = [] }) => {
    const router = useRouter();

    const DeleteHandler = async (id) => {
        const { success, error, message } = await Actions.Blog.Delete(token, id);
        if (success) {
    ...
    const EditHandler = async (id) => {
        router.push(`${NavLink.admin.blogs}?id=update&q_id=${id}`);
    }

    const DetailsHandler = async (id) => {
        router.push(`${NavLink.admin.blogs}?id=view&q_id=${id}`);
    }
