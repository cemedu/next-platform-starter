...
const NotificationTable = ({ rows = [], cols = [], token }) => {
    const router = useRouter();

    const DeleteHandler = async (id) => {
        const { success, error, message } = await Actions.Notification.Delete(token, id);
        if (success) {
    ...
    const EditHandler = async (id) => {
        router.push(`${NavLink.admin.notifications}?id=update&q_id=${id}`);
    }

    const DetailsHandler = async (id) => {
        router.push(`${NavLink.admin.notifications}?id=view&q_id=${id}`);
    }
